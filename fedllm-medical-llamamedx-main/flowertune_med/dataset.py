...
def load_data(partition_id: int, num_partitions: int, dataset_name: str):
    """Load partition data for Simulation mode (uses flwr_datasets)."""
...
def load_local_data(data_path: str):
    """Load partition data for Deployment mode (from disk, created by flwr-datasets create)."""
    client_trainset = load_from_disk(data_path)
    client_trainset = reformat(client_trainset, llm_task="medical")
    return client_trainset
