# FlowerTune LLM on Medical Dataset (Llama-MedX)

Federated instruction tuning with a pretrained [skumar9/Llama-medx_v3.2](https://huggingface.co/skumar9/Llama-medx_v3.2) model on a [Medical dataset](https://huggingface.co/datasets/medalpaca/medical_meadow_medical_flashcards). This app uses [Flower Datasets](https://flower.ai/docs/datasets/) to download, partition, and preprocess the dataset.

## Quickstart

```
flwr new @heart-ai-lab/flowertune-med-llamamedx
```
...
```
cd flowertune-med-llamamedx && pip install -e .
```
...
- **LoRA Configuration**: Rank (r): 32, Alpha: 64, Target Modules: q_proj, k_proj, v_proj, o_proj, gate_proj, up_proj, down_proj
- **Training Configuration**: Batch size: 16, Max steps: 6, Warmup steps: 2, Total rounds: 20, Fraction fit per round: 0.15
