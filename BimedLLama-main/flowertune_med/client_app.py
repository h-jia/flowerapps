...
def client_fn(context: Context) -> FlowerClient:
    ...
    # Distinguish between Simulation and Deployment using context.node_config
    if (
        ...
        client_trainset = load_data(
            partition_id, num_partitions, cfg.static.dataset.name
        )
    else:
        # Deployment Engine: load demo data or real user data from disk
        data_path = context.node_config["data-path"]
        client_trainset = load_local_data(data_path)
        num_partitions = 1  # Not used in deployment, placeholder for FlowerClient
